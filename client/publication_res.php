...
$url_temporaneo = HOST . PORTA . "/Colibrary/publications/mashup/";
...
$url_origine = HOST . PORTA . "/ColibraryClient/publications.php";
...
//get the SocialType2 parameter
if(isset($_GET['SocialType2'])) {
	if($_GET['SocialType2'] != NULL) {
		$social_type = $_GET['SocialType2'];		
    }
}

//get the TagSource2 parameter
if(isset($_GET['TagSource2'])) {
	if($_GET['TagSource2'] != NULL) {
		$tag_source = $_GET['TagSource2'];		
    }
}

//get the ReviewSource2 parameter
if(isset($_GET['ReviewSource2'])) {
	if($_GET['ReviewSource2'] != NULL) {
		$rev_source = $_GET['ReviewSource2'];		
    }
}

//get the UserSource2 parameter
if(isset($_GET['UserSource2'])) {
	if($_GET['UserSource2'] != NULL) {
		$user_source = $_GET['UserSource2'];		
    }
}

//get the tag_limit2 parameter
if(isset($_GET['tag_limit2'])) {
	if($_GET['tag_limit2'] != NULL) {
		$tag_limit = $_GET['tag_limit2'];		
    }
}

/* echo "Parameters<br/><br/>";
echo "Interash: " . $interhash . "<br/>";
echo "Data Type: " . $datatype . "<br/>";
echo "Social Type: " . $social_type . "<br/>";
echo "Tag Source: " . $tag_source . "<br/>";
echo "Review Source: " . $rev_source . "<br/>";
echo "User Source: " . $user_source . "<br/>";
echo "Tag Limit: " . $tag_limit . "<br/>";

 */

$url = $url_temporaneo . $interhash;
...
if ($model->isEmpty()) {
	...
	$rdf_url = RDF_REPOSITORY . $interhash . "_biblio.rdf";
	...
	while ($it->hasNext()) {
		...
		switch($statement->getLabelPredicate() ) {
			...
			case 'http://purl.org/dc/elements/1.1/description':
				$description = $statement->getLabelObject();
				break;
			...
			case 'http://purl.org/dc/elements/1.1/contributor':
				$editor = $statement->getLabelObject(); 
				break;
				
			case 'http://collab.di.uniba.it/colibrary/vocab/journal':
				$journal = $statement->getLabelObject(); 
				break;
				
			case 'http://collab.di.uniba.it/colibrary/vocab/link':
				$temp = $statement->getLabelObject(); 
				if (strpos($temp, 'bibsonomy') != false)
					$bibsonomy_link = $temp;
				elseif (strpos($temp, 'citeulike') != false)
					$citeulike_link = $temp;
				elseif (strpos($temp, 'acm') != false)
					$acm_link = $temp;
				break;
				
			case 'http://purl.org/dc/elements/1.1/identifier':
				$temp = $statement->getLabelObject(); 
				if (strpos($temp, 'DOI') != false)
					$doi = str_replace('urn:DOI:', '', $temp);
				elseif (strpos($temp, 'ISBN') != false)
					$isbn = str_replace('urn:ISBN:', '', $temp);
				elseif (strpos($temp, 'ISSN') != false)
					$issn = str_replace('urn:ISSN:', '', $temp);
				break;
		}
	}
	
	if ($datatype != 'biblio') {
		if ($social_type == 'tags' || $social_type == 'both') {
			$url_tags = $url . "/social/tags";
			//if($tag_source != null && $tag_source != 'all')
			//	$url_tags .= '/' . $tag_source; 
			$model = ModelFactory::getDefaultModel();
			$model->load($url_tags);
		...
		if ($social_type == 'reviews' || $social_type == 'both') {
			$url_reviews = $url . "/social/reviews";
			//if($rev_source != null && $rev_source != 'all')
			//	$url_reviews .= '/' . $rev_source;
			$model = ModelFactory::getDefaultModel();
		...
		if ($social_type == 'users' || $social_type == 'both') {
			$url_users = $url . "/social/users";
			//if($user_source != null && $user_source != 'all')
			//	$url_users .= '/' . $user_source;
			$model = ModelFactory::getDefaultModel();
...
var loading_user= "Retrieving Users...";
var get_interhash=$.getURLParam("interhash");
...
$('#a_tit_t').click(function(){
			...
			$('#box_tag').load('tags_req_pubs.php?interhash='+get_interhash);
...
$('#a_tit_r').click(function(){
			...
			$('#box_rev').load('reviews_req_pubs.php?interhash='+get_interhash);
			$(this).css({ "background-color" : "#FF9900" });
...
// carico con ajax gli users ed una volta ricevuti li mostro e li nascondo senza rifare la richiesta
						           
$('#a_tit_u').click(function(){
            if($(this).html()=='Search for Users'){
            var loading = "<p align='center'><img src='img/ajax-loader.gif'> "+loading_user+"</p>";
			$('#box_user').html(loading);	
			$('#box_user').load('users_req_pubs.php?interhash='+get_interhash);
			$(this).css({ "background-color" : "#FF9900" });
			$(this).html('User List');
			var vis2=true;
			}
...
<div id="colibrary_output2" >
	...
	<ul id="menu2" class="menu noaccordion ">
		...
		<li id='li_tag'>
			...
			<ul>
			<li><div id="box_tag"><?php include("tags_pubs.php"); ?></div></li>	
			</ul>
		</li>
		<?php } ?>
		...
		<?php if($_SESSION['user_flag']) {?>
		<li>
			<a id='a_tit' href="#">User List</a>
			<ul>
			<li><div id="box_user"><div id='content_users_req'><?php include("users_pubs.php"); ?></div></div></li>
			</ul>
		</li>
		<?php }?>	
        ...
        <?php if (!$_SESSION['user_flag']){ ?>
		<li>
			<a id='a_tit_u' href="#">Search for Users</a>
			<ul>
			<li><div id="box_user"></div></li>
        	</ul>
		</li>
		<?php }?>		
	</ul>	
...
<div id="colibrary_output" >
	...
	<ul id="menu" class="menu noaccordion expandfirst">
		<li>
			...
			<ul>
			<li><div id="box_bib"><?php if ($request_valid) include("biblio_pubs.php"); else print "Invalid Interash";?></div></li>
			</ul>
		</li>
		...
		<li>
			...
			<ul>
			<li><div id="box_rev"><div id='content_reviews_req'><?php include("reviews_pubs.php"); ?></div></div></li>
			</ul>
		</li>
		<?php }?>
