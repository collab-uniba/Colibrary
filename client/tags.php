...
function makeTagCloud($tags, $source){
	...
	foreach ($tags as $key => $occ) {
		...
		if (strpos($source[$key], 'anobii') != false) {	
			$color = 'red';
		}
		elseif (strpos($source[$key], 'amazon') != false) {
			$color = '#0B479D';			
		}
		elseif (strpos($source[$key], 'librarything') != false) {
			$color = 'green';
		}			
...
//---------TAG SECTION----------------------
	...
	{?>
			 ...
			 <tr>
			 <td>
			 <span id="formInfo" title='<div>Clicking on blue tags you will be redirected to tags in Amazon</div><br><div>Clicking on red tags you will be redirected to tags in Anobii</div><br><div>Clicking on green tags you will be redirected to tags in Library Thing</div><br>'>
                <img src="img/info.gif" border="0" /></a>
	         </span></td>
			 <td>
			 <div ><a href="<?php echo RDF_REPOSITORY . $isbn . '_tags.rdf'; ?>" target="_blank">
